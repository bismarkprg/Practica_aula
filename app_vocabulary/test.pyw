...
class JuegoVocabulario:
    ...
    def establecer_estilos(self):
        style = ttk.Style()
        style.configure("Rounded.TButton", font=("Arial", 12), padding=8, relief="raised", borderwidth=2)
        style.map("Rounded.TButton",
                  foreground=[("active", "#000")],
                  background=[("active", "#add8e6")])  # celeste al hacer hover
        style.configure("Disabled.TButton", background="#add8e6", foreground="#000")
    ...
    def cargar_siguiente_lote(self):
        ...
        for palabra in self.palabras_es:
            btn = ttk.Button(self.frame_es, text=palabra, style="Rounded.TButton",
                             command=lambda p=palabra: self.seleccionar_es(p))
            btn.pack(pady=6)
        ...
        for palabra in self.palabras_en:
            btn = ttk.Button(self.frame_en, text=palabra, style="Rounded.TButton",
                             command=lambda p=palabra: self.seleccionar_en(p))
            btn.pack(pady=6)
    ...
    def seleccionar_en(self, palabra):
        if self.palabra_seleccionada:
            ...
            elif all(btn.instate(['disabled']) for btn in self.botones_es):
                self.cargar_siguiente_lote()
